...
class _LiteTouchTransport:
    ...
    async def _connect(self) -> None:
        _LOGGER.debug(f"Open Connect: {self.host}, {self.port}")
        self._reader, self._writer = await asyncio.open_connection(self.host, self.port)
        _LOGGER.debug(
            f"Response from asynco litetouch: {self._reader} - {self._writer}"
        )
        _LOGGER.info("[%s] connected to %s:%s", self.name, self.host, self.port)

    async def _run(self) -> None:
        _LOGGER.info("Connecting to LiteTouch Server")
        delay = self.reconnect_min_delay
        while not self._stop.is_set():
            try:
                if not self.is_connected:
                    _LOGGER.debug(
                        f"Not Connected, try to connect, {self.reconnect_min_delay}"
                    )
                    await self._connect()
                    delay = self.reconnect_min_delay
